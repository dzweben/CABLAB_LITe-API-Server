...
google_output_csv = "google_sheet_data.csv"
redcap_api_url = "https://cphapps.temple.edu/redcap/api/"
redcap_api_token = "6E4C4AEFF6A66B2AC62EFCB8BB9246D5"
redcap_report_id = 8162
redcap_output_csv = "redcap_report_data.csv"
intersection_csv = "intersection_data.csv"

# Set the time interval for filtering (in days)
date_filter_days = 30  # Change this value to adjust the time length (e.g., 60 for two months)
...
def prepare_redcap_import(intersection_data):
    ...
    for _, row in intersection_data.iterrows():
        ...
        next_month_date = base_date + timedelta(days=31)
        new_date = next_month_date.replace(day=20)  # Set to 20th day of the next month
        new_date = new_date.replace(hour=19, minute=0, second=0, microsecond=0)  # Set time to 7 PM
        new_date = new_date.strftime('%Y-%m-%d %H:%M:%S')  # Format as YYYY-MM-DD HH:MM:SS


        
        record = {
            "record_id": record_id,
            "redcap_event_name": "screen_time_y1_arm_1",  # Specify the event name here
            "screen_time_1_1_date": new_date,
            "screen_time_cycle_1": 1,
            "screen_time_settings_complete": 2,
        }
