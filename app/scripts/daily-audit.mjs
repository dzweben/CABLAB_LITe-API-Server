...
const main = async () => {
  ...
  if (fetchAgeMin != null && fetchAgeMin > 120) warnings.push(`data ${fetchAgeMin} min stale`);
  ...
  const entry = { date: AUDIT_DATE, verdict, problems: problems.length, warnings: warnings.length, generatedAt: report.generatedAt };
  ...
  console.log(`Audit ${AUDIT_DATE}: ${verdict}${problems.length ? " — " + problems.join("; ") : ""}${warnings.length ? " | warn: " + warnings.join("; ") : ""}`);
};
