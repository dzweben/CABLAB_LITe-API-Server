...
import type { Participant, WaveYear, SentLogEntry, CompletionCode } from "@/types";
...
// Follow-up tracker: focused on the V1 → at-home survey → V2 lifecycle.
// STS schedule + EMA prompt timing live on their own dedicated pages.
...
export default function FollowupPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "outstanding" | "v2_done" | "v1_only" | "athome_pending">("all");
  ...
  const rows = useMemo(() => {
    ...
    if (statusFilter === "v2_done") {
      ...
      xs = xs.filter(p => !p.waves[wave]?.v2?.allComplete);
    } else if (statusFilter === "athome_pending") {
      xs = xs.filter(p => {
        const ah = p.waves[wave]?.atHome;
        return ah && ah.timestamp && ah.athomeMeasuresComplete !== 2;
      });
    }
    if (search.trim()) {
  ...
  const stats = useMemo(() => {
    ...
    const athomeStarted = scoped.filter(p => p.waves[wave]?.atHome?.timestamp).length;
    const athomeDone = scoped.filter(p => p.waves[wave]?.atHome?.athomeMeasuresComplete === 2).length;
    return { inWave, v1Done, v2Done, athomeStarted, athomeDone };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <Stat label="At-home started" value={stats.athomeStarted} suffix={`/${stats.inWave}`} color="amber" />
        <Stat label="At-home complete" value={stats.athomeDone} suffix={`/${stats.inWave}`} color="indigo" />
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <option value="athome_pending">At-home started, not finished</option>
        </select>
        <input
          type="text"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-gray-600">
                  ...
                  <th className="text-left px-3 py-3 font-semibold">At-home started</th>
                  <th className="text-center px-3 py-3 font-semibold">At-home complete</th>
                  <th className="text-center px-3 py-3 font-semibold">At-home sends</th>
                  ...
                  return (
                    <React.Fragment key={p.pid}>
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 text-gray-700 text-xs font-mono">
                          {ah?.timestamp ? formatDateTime(ah.timestamp) : "—"}
                        </td>
                        <td className="px-3 py-2.5 text-center">
                          <CompletionPill code={ah?.athomeMeasuresComplete} />
                        </td>
                        <td className="px-3 py-2.5 text-center font-mono text-xs">
                          {athomeSends || <span className="text-gray-300">·</span>}
                        </td>
                        <td className="px-3 py-2.5 text-center">
                          {w.v2?.allComplete
                            ? <CheckWith label={String(sheet?.v2Date || w.v2?.date || "").slice(0, 10)} />
                            : <Dash />}
                        </td>
...
function CheckWith({ label }: { label: string }) {
  return (
    <div className="inline-flex flex-col items-center gap-0.5">
      <Check />
      {label && <span className="text-[10px] text-emerald-700 font-mono">{label}</span>}
    </div>
  );
}

function CompletionPill({ code }: { code: CompletionCode | undefined }) {
  if (code === 2) return <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 text-xs">✓</span>;
  if (code === 1) return <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-amber-100 text-amber-700 text-[10px] font-semibold">unv</span>;
  if (code === 0) return <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-gray-100 text-gray-500 text-[10px] font-semibold">·</span>;
  return <span className="text-gray-300">—</span>;
}
...
function DetailPane({ p, wave, sent }: { p: Participant; wave: WaveYear; sent: SentLogEntry[] }) {
  ...
  // At-home section-by-section completion if the report gave us per-form codes.
  const ahForms = ah?.formsFromReport ? Object.entries(ah.formsFromReport).filter(([, v]) => v != null) : [];
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-sm">
      <div>
        ...
        <dl className="space-y-1">
          ...
          {sheet?.raTag && <KV label="RA tag" value={String(sheet.raTag)} />}
      ...
      <div>
        ...
        <dl className="space-y-1">
          ...
          <KV label="At-home start (timestamp)" value={ah?.timestamp ? formatDateTime(ah.timestamp) : "—"} />
          <KV label="At-home break_1 complete" value={ah?.break1Complete === 2 ? "✓" : "—"} />
          <KV label="At-home full survey complete" value={ah?.athomeMeasuresComplete === 2 ? "✓" : "—"} />
        ...
        {ahForms.length > 0 && (
          <>
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mt-4 mb-2">At-home sections ({ahForms.filter(([,v]) => v === 2).length}/{ahForms.length})</h4>
            <div className="grid grid-cols-2 gap-1">
              {ahForms.map(([name, code]) => (
                <div key={name} className="flex items-center gap-2 text-xs">
                  <CompletionPill code={code} />
